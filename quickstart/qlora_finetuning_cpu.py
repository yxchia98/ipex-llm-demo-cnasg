...
if __name__ == "__main__":
    ...
    def prompt_formatter(dataset):
        ...
        full_prompt = f"<|im_start|>system\nYou are a helpful medical assistant.<|im_end|>\n<|im_start|>user\n{dataset['Prompt']}<|im_end|>\n<|im_start|>assistant\n{dataset['Completion']}<|im_end|>"
        # result = {}
        # result['text'] = tokenizer(full_prompt, padding="max_length", truncation=True)
        return tokenizer(full_prompt, padding="max_length", truncation=True)

    train_data = data.map(prompt_formatter, remove_columns=['Prompt', 'Completion'])
    print(train_data)
    # print(train_data[50]['text'])
